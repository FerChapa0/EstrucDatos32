...
def menu_consultas_clientes(conn):
    with conn:
        while True:
...
def exportar_reporte(data, tipo_reporte, fecha_inicio, fecha_fin):
    try:
        ...
        if opcion_exportar == "sí" or opcion_exportar == "si":
            ...
            # Exportar a Excel
            df = padas.DataFrame(data, columns=[data[0] for data in data[0]])
...
def estadisticas_clientes_con_mas_notas(conn):
    with conn:
        try:
...
def estadisticas_promedio_montos_notas(conn):
    with conn:
        try:
            fecha_inicio = input("Ingrese la fecha inicial del período a reportar (YYYY-MM-DD): ")
            fecha_fin = input("Ingrese la fecha final del período a reportar (YYYY-MM-DD): ")
...
def estadisticas_servicios_mas_prestados(conn):
    with conn:
        try:
            # pedir info
