...
def consulta_periodo():
    try:
        fecha_inicio = input("Ingrese la fecha de inicio (DD-MM-YYYY, o presione Enter para usar 01/01/2000): ")
        if fecha_inicio:
            fecha_inicio = datetime.strptime(fecha_inicio, "%d-%m-%Y").strftime("%Y-%m-%d")
        else:
            fecha_inicio = "2000-01-01"

        fecha_fin = input("Ingrese la fecha de fin (DD-MM-YYYY, o presione Enter para usar la fecha actual): ")
        if fecha_fin:
        ...
        if opcion_exportar == "sí" or opcion_exportar == "si":
            ...
            df = pd.DataFrame(notas, columns=['Folio', 'Fecha', 'Cliente', 'Monto'])
            df.to_csv(nombre_archivo, index=False)
            print(f"Reporte exportado exitosamente como {nombre_archivo}")
    except Exception as e:
        print(f"Error al consultar notas por período: {e}")
...
def consulta_folio():
    try:
        ...
        notas_tabular = cursor.fetchall()

        if not notas_tabular:
        ...
        for nota_tabular in notas_tabular:
            print(f"{nota_tabular[0]}\t{nota_tabular[1]}\t{nota_tabular[2]}")
        ...
        if nota:
            ...
            if servicios:
                ...
                for servicio in servicios:
                    print(f"Servicio: {servicio['nombre']}, Costo: ${servicio['costo']}")
            else:
                print("No hay servicios asociados a esta nota.")
        else:
            print(f"La nota con folio {folio_nota} no se encuentra en el sistema o está cancelada.")
...
def estadisticas_clientes_con_mas_notas():
  try:
      ...
      fecha_inicio = input("Ingrese la fecha inicial del período a reportar (DD-MM-YYYY): ")
      fecha_inicio = datetime.strptime(fecha_inicio, "%d-%m-%Y").strftime("%Y-%m-%d")
